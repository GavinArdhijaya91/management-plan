...
test.describe.serial('private workspace journey', () => {
  ...
  test('a confirmed account can create its first workspace and business plan', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/profile\?success=/)
...
<    await expect(page).toHaveURL(/\/management\?success=/)
    ...
    const actualComposer = page.locator('details').filter({ hasText: '3. Catat actual' })
    await actualComposer.getByLabel('Target terukur').selectOption({ label: 'Naikkan omzet E2E - Omzet E2E' })
    await actualComposer.getByLabel('Nilai aktual').fill('1500000')
    await actualComposer.getByLabel('Waktu pengukuran').fill('2026-08-25T09:00')
    await actualComposer.getByLabel('Sumber').fill('Rekap kas E2E')
