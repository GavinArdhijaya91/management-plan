...
import { ProfileEditor } from '@/app/profil/_components/profile-editor'
...
export default async function ProfilePage({ searchParams }: ProfilePageProps) {
  const [user, feedback] = await Promise.all([requireAuthenticatedUser('/profil'), searchParams])
  const supabase = await createClient()
  const [profileResult, preferenceResult] = await Promise.all([
    supabase
      .from('profiles')
      .select('full_name,display_name,email,phone,bio,avatar_path,profile_banner_path,headline,status_text')
      .eq('user_id', user.id)
      .maybeSingle(),
    supabase
      .from('profile_preferences')
      .select('locale,timezone,date_format,theme,calendar_notifications,review_notifications,show_activity_status')
      .eq('user_id', user.id)
      .maybeSingle(),
  ])
    ...
    <main className="app-shell">
      ...
      <div className="page-shell motion-page-enter max-w-3xl">
        ...
        {profileResult.error || preferenceResult.error ? (
          ...
          <>
            {profile && (
              <ProfileEditor profile={profile} showActivityStatus={preferences?.show_activity_status ?? true} />
            )}
