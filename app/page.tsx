...
import { landingCopy } from '@/app/_i18n/pages/landing'

const featureIcons = [WalletCards, CalendarDays, TrendingUp, BarChart3] as const

export default function Home() {
  const { locale } = useLanguage()
  const copy = landingCopy[locale]
  const businessPulses = [
    { label: copy.preview.todaySalesLabel, value: copy.preview.todaySalesValue, note: copy.preview.todaySalesNote },
    { label: copy.preview.capitalLabel, value: copy.preview.capitalValue, note: copy.preview.capitalNote },
    { label: copy.preview.agendaLabel, value: copy.preview.agendaValue, note: copy.preview.agendaNote },
  ]
  return (
      ...
      <section className="border-b border-zinc-200/80">
        <div className="mx-auto grid max-w-7xl gap-14 px-4 py-16 md:px-6 md:py-24 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,.95fr)] lg:items-center lg:py-28">
          <Reveal className="min-w-0">
            <p className="app-label mb-6">{copy.hero.eyebrow}</p>
            <h1 className="min-w-0 max-w-4xl text-[clamp(3.25rem,6.4vw,5.75rem)] font-semibold leading-[0.94] tracking-[-0.055em]">
              ...
              <span className="mt-2 block min-h-[1.05em] text-zinc-500">
                <TypewriterText items={copy.hero.typewriter} />
              </span>
            </h1>
            <p className="mt-7 max-w-xl text-base leading-7 text-zinc-600 md:text-lg">{copy.hero.description}</p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Link href="/demo/dashboard" className="app-button motion-press px-6">
                {copy.hero.openDemo} <ArrowRight className="size-4" />
              </Link>
              <a
                href="#fitur"
          ...
          <Reveal delay={120} className="app-card min-w-0 overflow-hidden lg:self-center">
            <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4 md:px-6">
              <div>
                <p className="app-label">{copy.preview.date}</p>
                <h2 className="mt-1 text-xl font-semibold">{copy.preview.greeting}</h2>
              </div>
              <span className="grid size-10 place-items-center rounded-full bg-zinc-950 text-sm font-semibold text-white">
                {copy.preview.initials}
              </span>
            </div>
            <div className="p-4 md:p-6">
              <div className="rounded-lg bg-zinc-950 p-5 text-white">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="app-label text-zinc-400">{copy.preview.profitLabel}</p>
      ...
      <section id="fitur" className="scroll-mt-24 px-4 py-20 md:px-6 md:py-28">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-12 max-w-3xl">
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-zinc-600">{copy.featuresSection.description}</p>
          </Reveal>
          <div className="grid gap-4 md:grid-cols-2">
            {copy.features.map((feature, index) => {
              const Icon = featureIcons[index] ?? WalletCards
              return (
        ...
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.85fr_1.15fr] lg:items-start">
          ...
          <Reveal delay={100}>
            <ol className="divide-y divide-zinc-200 border-y border-zinc-200">
              {copy.howItWorks.steps.map(({ verb, title, description }) => (
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-10 border-b border-zinc-200 pb-10 md:grid-cols-[minmax(0,1fr)_auto_auto] md:gap-16">
            <div className="max-w-md">
              ...
              <p className="mt-4 text-sm leading-6 text-zinc-500">{copy.footer.tagline}</p>
            </div>
            <div>
              <p className="app-label mb-4">{copy.footer.account}</p>
            ...
            <div>
              <p className="app-label mb-4">{copy.footer.information}</p>
