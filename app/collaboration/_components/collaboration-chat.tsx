...
import type {
  ...
  ChatMessage,
  ChatReaction,
} from '@/app/collaboration/_lib/chat-types'
import { hasExpectedFileSignature, sanitizeAttachmentName } from '@/app/collaboration/_lib/chat-file-security'
...
export function CollaborationChat({
  ...
  const startDirect = async (targetUserId: string) => {
    ...
    router.push(`/collaboration?conversation=${data}`)
    router.refresh()
  }
      ...
      <aside className="hidden overflow-y-auto border-r border-zinc-200 bg-[#fafafa] p-3 lg:block">
        ...
        <nav className="mt-2 space-y-1">
          {channels.map((conversation) => (
            <Link
              key={conversation.id}
              href={`/collaboration?conversation=${conversation.id}`}
          ...
          <>
            <header className="flex min-h-16 items-center border-b border-zinc-200 px-4">
              ...
              <select
                ...
                onChange={(event) => router.push(`/collaboration?conversation=${event.target.value}`)}
