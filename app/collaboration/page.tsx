import { CollaborationChat } from '@/app/collaboration/_components/collaboration-chat'
import type {
  ChatAttachment,
  ChatConversation,
...
export default async function CollaborationPage({ searchParams }: CollaborationPageProps) {
  const [user, workspace, query] = await Promise.all([
    requireAuthenticatedUser('/collaboration'),
    requireActiveWorkspace('/collaboration'),
    searchParams,
  ])
