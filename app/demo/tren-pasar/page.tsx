...
export default function TrenPasarPage() {
  const [products, setProducts] = useLocalStorage<DemoMarketProduct[]>('siapin:demo:market-products', initialProducts)
  ...
  const save = (event: React.FormEvent) => {
    ...
    setModalOpen(false)
    setToast(editingId === null ? 'Produk pantauan ditambahkan.' : 'Asumsi tren diperbarui.')
  }
  const reset = () => {
    setProducts(initialProducts)
    setToast('Data tren demo berhasil dikembalikan.')
  }
  ...
  return (
    <main className="app-shell">
      ...
      <div className="page-shell motion-page-enter">
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <h1 className="app-heading">Analisis tren pasar</h1>
            <p className="mt-2 text-zinc-500">Uji asumsi produk dan lihat prioritas tindakan yang dihasilkan.</p>
          </div>
        ...
        <DemoDataNotice>
          Data tren merupakan simulasi edukatif, bukan rekomendasi investasi atau riset pasar aktual.
        </DemoDataNotice>
        ...
        <SalesChart
          data={visibleTrend}
          type="line"
          title={`Tren penjualan, ${period} bulan terakhir`}
          variant="monochrome"
        />
        <section className="my-6 grid gap-4 md:grid-cols-2">
          <div className="app-card p-5">
            <TrendingUp className="size-5" />
            <h2 className="mt-4 font-serif text-lg font-semibold">Peluang utama: {strongest?.name}</h2>
            <p className="mt-1 text-sm text-zinc-500">
              Pertumbuhan simulasi {strongest?.change ?? 0}%. Validasi permintaan sebelum menambah stok.
            </p>
          </div>
          <div className="app-card p-5">
            <AlertCircle className="size-5 text-zinc-500" />
            <h2 className="mt-4 font-serif text-lg font-semibold">Perlu evaluasi: {weakest?.name}</h2>
            <p className="mt-1 text-sm text-zinc-500">
              Perubahan {weakest?.change ?? 0}%. Periksa harga, promosi, dan relevansi produk.
            </p>
          </div>
        </section>
        <section className="app-card overflow-hidden">
          <div className="border-b border-zinc-100 p-5">
            <p className="app-label">Asumsi produk</p>
            <h2 className="mt-1 font-serif text-xl font-semibold">Produk yang dipantau</h2>
          </div>
          {products.length === 0 ? (
            <div className="p-10 text-center text-sm text-zinc-500">Belum ada produk pantauan.</div>
          ) : (
        ...
        <div className="mt-6 rounded-2xl bg-zinc-950 p-5 text-white">
          <div className="flex gap-3">
            ...
            <div>
              <h2 className="font-serif font-semibold">Cara memakai halaman ini</h2>
              <p className="mt-1 text-sm text-zinc-400">
                Masukkan perubahan yang Anda amati, bandingkan produk, lalu jadikan hasilnya hipotesis untuk diuji
                melalui promosi kecil.
              </p>
            </div>
          </div>
        </div>
      ...
      <Modal
        ...
        title={editingId === null ? 'Tambah produk pantauan' : 'Edit asumsi produk'}
      >
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="app-button">Simpan</button>
          </div>
        </form>
      </Modal>
