...
export async function createPrivateTransactionAction(formData: FormData) {
  ...
  if (!parsed.success) {
    redirect(
      `/management?error=${encodeURIComponent(parsed.error.issues[0]?.message ?? 'Data transaksi tidak valid.')}`,
    )
  }
  ...
  if (error) {
    ...
    redirect(`/management?error=${encodeURIComponent('Transaksi gagal disimpan. Periksa akses atau coba kembali.')}`)
  }

  revalidatePath('/management')
  revalidatePath('/planning/metrics')
  revalidatePath('/dashboard')
  redirect(`/management?success=${encodeURIComponent('Transaksi berhasil ditambahkan.')}`)
}
