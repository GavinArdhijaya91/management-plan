...
export async function saveCommunityDraft(input: CommunityDraftInput): Promise<CommunityActionResult> {
  ...
  const [user, workspace] = await Promise.all([
    requireAuthenticatedUser('/community/create'),
    requireActiveWorkspace('/community/create'),
  ])
...
export async function publishCommunityPost(postId: string): Promise<CommunityActionResult> {
  const workspace = await requireActiveWorkspace('/community/create')
  await requireAuthenticatedUser('/community/create')
  const supabase = await createClient()
...
export async function archiveCommunityPost(postId: string, reason: string): Promise<CommunityActionResult> {
  await requireAuthenticatedUser(`/community/${postId}`)
  await requireActiveWorkspace(`/community/${postId}`)
  const supabase = await createClient()
...
export async function deleteCommunityDraft(postId: string): Promise<CommunityActionResult> {
  const user = await requireAuthenticatedUser('/community/my-posts')
  await requireActiveWorkspace('/community/my-posts')
  const supabase = await createClient()
  const { error } = await supabase
    .from('community_posts')
    .delete()
