...
export default async function MyCommunityPostsPage({
  ...
  const [user, workspace, params] = await Promise.all([
    requireAuthenticatedUser('/komunitas/post-saya'),
    requireActiveWorkspace('/komunitas/post-saya'),
    searchParams,
  ])
        ...
        <div className="mt-6 flex flex-wrap gap-2" aria-label="Tampilan post">
          {!workspaceArchive &&
            statuses.map(([value, label]) => (
              <Link
                key={value}
                href={`/komunitas/post-saya?status=${value}`}
              ...
              {result.posts.map((post) => (
                <div key={post.id}>
                  {post.publication_status === 'draft' ? (
                    <Link
                      href={`/komunitas/buat?id=${post.id}`}
