...
## Before you start you must use 3 quick reads
...
- For **small fixes** (typo, small bug), you can go straight to a pull request or just link the issue if there is one.
...
Run these if they are the same checks that run in CI:
...
- A maintainer will review when they can. Be patient because we are a small team.
- You may get kind, direct feedback. It is not personal because we want the change to be safe and clear for everyone.
...
- Use **fake data** for development and tests dont use real business data or real emails
- Keep workspace data isolated, so one workspace must never see another workspace's data
...
- Hard-delete community posts or reviews through the browser can be used the proper lifecycle (archive, etc.)
